...
# def upload_to_qdrant(chunks, metadata):
#     """Upload chunks to Qdrant with safe point IDs."""
#     points = []
#     file_path = metadata.get("file_path", "unknown")
#     for i, chunk in enumerate(chunks):
#         embedding = get_embedding(chunk)
#         # Safe alphanumeric point ID
#         safe_file_name = re.sub(r'[^a-zA-Z0-9_-]', '_', os.path.basename(file_path))
#         point_id = f"{safe_file_name}_{i}"
#         points.append(
#             models.PointStruct(
#                 id=point_id,
#                 vector=embedding,
...
def upload_to_qdrant(chunks, metadata):
    ...
    for chunk in chunks:
        embedding = get_embedding(chunk)
        point_id = str(uuid.uuid4())  # <-- Must use UUID now!
...
def embed_book_content():
    """Main function to read, chunk, embed, and upload book content."""
    base_docs = os.getenv("BOOK_DOCS_PATH", "frontend/docs/")
    markdown_files = collect_markdown_files(base_docs)
    if not markdown_files:
        print("No markdown files found at", base_docs)
        return
